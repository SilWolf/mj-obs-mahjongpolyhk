...
import { getTournamentTeamsWithStatistics } from '@/helpers/sanity.helper'
  ...
  if (slide.type === 'teams') {
    ...
    return (
      ...
      >
        ...
        >
          <div className="flex items-center text-[0.75em]">
            ...
            {[0, 1, 2, 3, 4, 5, 6, 7].map((ranking) => (
              <div
                ...
                style={{
                  ...
                  opacity: slide.teams[ranking].newResult ? 1 : 0.3,
                }}
              >
                <img
                  src={
                ...
                <div className="absolute left-[0em]">
                  {/* {slide.teams[ranking].ranking > ranking + 1 && (
                      <span>
                ...
                <p className="flex-1 text-center space-x-1 text-[1.25em]">
                  <span>{ranking + 1}</span>
                </p>
                <p className="flex-[2.5] overflow-hidden">
                  <span>{slide.teams[ranking].team.name}</span>
                </p>
                <p
                  className={cns('flex-1 text-right', {
                    'text-green-500':
                      slide.teams[ranking].newResult &&
                      slide.teams[ranking].newResult!.point >
                        slide.teams[ranking].point,
                    'text-red-500':
                      slide.teams[ranking].newResult &&
                      slide.teams[ranking].newResult!.point <
                        slide.teams[ranking].point,
                ...
                <p className="flex-1 text-right">
                  {ranking > 0
                    ? (
                        (slide.teams[ranking - 1].newResult?.point ||
                          slide.teams[ranking - 1].point ||
                          0) -
                        (slide.teams[ranking].newResult?.point ||
                          slide.teams[ranking].point ||
                          0)
                ...
                <p className="flex-1 text-right">
                  {slide.teams[ranking].newResult?.matchCount ||
                    slide.teams[ranking].matchCount}
...
const MatchSummaryPage = ({
  ...
  const { data: regularTeams } = useQuery({
    ...
    queryFn: () =>
      getTournamentTeamsWithStatistics(match?.tournament._id as string).then(
        (teams) => {
  ...
  const slides = useMemo<Slide[]>(() => {
      ...
      { type: 'empty', _id: 'empty', subslide: 0 },
      {
        ...
        teamAndPlayers: {
          playerEast: {
            ...
            result: {
              point: exportedMatch.result.players[0].point,
              ranking: exportedMatch.result.players[0].ranking,
            },
          },
          playerSouth: {
            ...
            result: {
              point: exportedMatch.result.players[1].point,
              ranking: exportedMatch.result.players[1].ranking,
            },
          },
          playerWest: {
            ...
            result: {
              point: exportedMatch.result.players[2].point,
              ranking: exportedMatch.result.players[2].ranking,
            },
          },
          playerNorth: {
            ...
            result: {
              point: exportedMatch.result.players[3].point,
              ranking: exportedMatch.result.players[3].ranking,
            },
          },
        },
        subslide: 1,
    ...
    const newTeams: {
      ...
      for (let i = 0; i < exportedMatch.result.players.length; i++) {
        if (team.team._id === match[`${playerKeys[i]}Team`]!._id) {
          return {
            _id: team.team._id,
            ...
            newResult: {
              point:
                ...
                exportedMatch.result.players[i].point +
                exportedMatch.result.players[i].penalty,
    ...
    resultSlides.push({
      ...
      teams: newTeams.sort(
        (a, b) =>
          (b.newResult?.point || b.point || 0) -
          (a.newResult?.point || a.point || 0)
      ),
      subslide: 1,
    })
